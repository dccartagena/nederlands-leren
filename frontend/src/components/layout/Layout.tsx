...
export default function Layout() {
  return (
    <div className="min-h-screen flex flex-col">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                    isActive
                      ? 'bg-dutch-100 text-dutch-700 dark:bg-dutch-900 dark:text-dutch-300'
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 z-30">
        <div className="flex">
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              key={to}
              ...
              className={({ isActive }) =>
                `flex-1 flex flex-col items-center py-2 text-xs font-medium transition-colors ${
                  isActive ? 'text-dutch-600 dark:text-dutch-400' : 'text-gray-500 dark:text-gray-500'
                }`
              }
            >
              <Icon size={20} />
