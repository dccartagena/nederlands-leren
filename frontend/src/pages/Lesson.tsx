      ...
      <section>
        ...
        {loadingVocab ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {vocab?.map(item => (
              <div key={item.id} className="p-3 rounded-xl bg-white border border-gray-200">
                <div className="font-semibold text-dutch-700">
                  {item.article ? `${item.article} ` : ''}{item.dutch_word}
                </div>
                <div className="text-sm text-gray-600">{item.spanish}</div>
                {item.example_nl && (
                  <div className="text-xs text-gray-400 mt-1 italic">{item.example_nl}</div>
                )}
              </div>
            ))}
          </div>
      ...
      <section>
        ...
        {loadingGrammar ? (
          ...
          <div className="space-y-3">
            {grammar?.map(topic => (
              <div key={topic.id} className="p-4 rounded-xl bg-white border border-gray-200">
                <div className="font-semibold">{topic.name_es}</div>
                {topic.description_es && (
                  <p className="text-sm text-gray-600 mt-1">{topic.description_es}</p>
                )}
              </div>
            ))}
            {grammar?.length === 0 && <p className="text-gray-400 text-sm">No hay temas de gramática aún.</p>}
          </div>
        )}
      </section>
    </div>
