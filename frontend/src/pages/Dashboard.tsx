...
const games = [
  { to: '/practice/flashcard', label: 'Tarjetas', icon: '🃏', color: 'bg-blue-50 hover:bg-blue-100 text-blue-700' },
  { to: '/practice/listen-choose', label: 'Escuchar', icon: '🎧', color: 'bg-purple-50 hover:bg-purple-100 text-purple-700' },
  { to: '/practice/word-match', label: 'Emparejar', icon: '🔗', color: 'bg-green-50 hover:bg-green-100 text-green-700' },
  { to: '/practice/multiple-choice', label: 'Test', icon: '❓', color: 'bg-yellow-50 hover:bg-yellow-100 text-yellow-700' },
  { to: '/practice/fill-blank', label: 'Rellenar', icon: '✏️', color: 'bg-orange-50 hover:bg-orange-100 text-orange-700' },
  { to: '/practice/unscramble', label: 'Ordenar', icon: '🔀', color: 'bg-red-50 hover:bg-red-100 text-red-700' },
  { to: '/practice/story', label: 'Historia', icon: '📖', color: 'bg-teal-50 hover:bg-teal-100 text-teal-700' },
]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Link to="/lesson" className="flex items-center gap-3 p-4 rounded-xl bg-white border border-gray-200 hover:border-dutch-500 transition-colors">
          <BookOpen className="text-dutch-600" size={22} />
          <div>
            <div className="font-medium">Lecciones</div>
            <div className="text-xs text-gray-500">Vocabulario y gramática</div>
          </div>
        </Link>
        <Link to="/chat" className="flex items-center gap-3 p-4 rounded-xl bg-white border border-gray-200 hover:border-dutch-500 transition-colors">
          <MessageCircle className="text-dutch-600" size={22} />
          <div>
            <div className="font-medium">Chat IA</div>
            <div className="text-xs text-gray-500">Practica conversación</div>
          </div>
        </Link>
      </div>
    </div>
